    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="absolute top-0 right-0 bottom-0 w-80 bg-[#0f0f0f]/95 backdrop-blur-md border-l border-glassBorder z-40 shadow-2xl flex flex-col"
        >
          <div className="flex items-center justify-between p-4 border-b border-glassBorder bg-[#0f0f0f] pb-0">
            <div className="flex gap-4">
              <button onClick={() => setActiveTab('history')} className={`pb-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'history' ? 'text-white border-blue-500' : 'text-gray-500 border-transparent hover:text-gray-300'}`}>
                History
              </button>
              <button onClick={() => setActiveTab('bookmarks')} className={`pb-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'bookmarks' ? 'text-white border-yellow-500' : 'text-gray-500 border-transparent hover:text-gray-300'}`}>
                Saved
              </button>
            </div>
                  ...
                  <div className="space-y-1">
                    {[...bookmarks].reverse().map((item, idx) => (
                      ...
                      >
                        ...
                        <div className="text-[10px] text-gray-500 font-mono truncate opacity-60">{item.url}</div>
                      </motion.button>
                    ))}
                  </div>
                )}
