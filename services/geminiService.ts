...
import { getCachedPage, cachePage } from "./cacheService";
...
export const generatePageContentStreaming = async (
  url: string,
  model: ModelTier,
  isDeepResearch: boolean = false,
  ...
  const cached = await getCachedPage(url, model, isDeepResearch).catch(() => null);
  if (cached) {
    onChunk(cached);
    return cached;
  }
  ...
  try {
    ...
    const finalHtml = cleanHtml(accumulated || "<!-- Error --><h1>Empty Response</h1>");

    cachePage(url, finalHtml, model, isDeepResearch).catch(() => {});
